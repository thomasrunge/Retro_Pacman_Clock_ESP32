...
#define EEPROM_DST 4
#define EEPROM_MSPACMAN 5
#define EEPROM_GHOST 6
...
// Time Variables
int8_t timezone; // offset to UTC
boolean dst; // has daylight saving time?
...
void setup() {
  ...
  touch.setCalibration(212, 1743, 1743, 266);
  ...
  sprintf(ipbuf, "GW:%d.%d.%d.%d", gw[0], gw[1], gw[2], gw[3]);
  ...
  timezone = EEPROM.read(EEPROM_TIMEZONE) - 12;
  if (timezone > 13 || timezone < -11) {
    timezone = 0;
  }

  if (EEPROM.read(EEPROM_DST) == 0) {
    dst = false;
  } else {
    dst = true;
  }
  ...
  NTP.begin("de.pool.ntp.org", timezone, dst);
...
void handle_ldr() {
  ...
  tft.fillRect(0, 0, 64, 36, TFT_BLACK);

  ldr = constrain(ldr, 400, 900);
  ldr = map(ldr, 400, 900, 0, 1020);
  ldr = constrain(ldr, 0, 1020);
...
void clocksetup() {
  ...
  // Timezone- and Alarm-labels
  drawString("TIMEZONE", 10,  60, false, color);
  drawString("ALARM",    40, 160, false, color);

  yield();

  // Timezone Set buttons
  tft.drawRoundRect(156, 35, 52, 20, RECTROUND, color); // timezone +
  drawString("+", 174, 37, false, color);
  tft.drawRoundRect(156, 80, 52, 20, RECTROUND, color); // timezone -
  drawString("-", 174, 81, false, color);
  // DST
  drawString("DST", 230, 35, false, color);
  tft.drawRoundRect(220, 58, 70, 20, RECTROUND, color); // DST on/off
  ...
  while (true) {
    ...
    if (t_dst == true) {
      drawString("ON ", 230, 60, true, color);
    } else {
      drawString("OFF", 230, 60, true, color);
    }
    ...
    tft.fillRect(156, 57, 52, 21, TFT_BLACK);
    int16_t x_c = 174;
    if (t_timezone >= 10) {
      x_c-= 8;
    }
    if (t_timezone < 0 && t_timezone > -10) {
      x_c-= 8;
    }
    if (t_timezone <= -10) {
      x_c-= 16;
    }
    drawNumber(t_timezone, x_c, 60, color);
    ...
    while (false == touch.isTouching()) {
      buzzer.update();
      if (last_sec != second() && !buzzer.isPlaying()) {
        // Display Current Date and Time
        drawString(NTP.getTimeDateString(), 5, 10, color);
        last_sec = second();
      }
      yield();
    }
    ...
    if ((xT >= 206) && (xT <= 319) && (yT >= 210) && (yT <= 239)) {
      ...
      break; // Exit setupmode
    } else if ((xT >= 151) && (xT <= 213) && (yT >= 30) && (yT <= 60)) {
      buzzer.tok();
      // Timezone +
      t_timezone++;
      if (t_timezone > 13) {
        // reset timezone to -11
        t_timezone = -11;
      }
    } else if ((xT >= 151) && (xT <= 213) && (yT >= 75) && (yT <= 105)) {
      buzzer.tok();
      // Timezone -
      t_timezone--;
      if (t_timezone < -11) {
        // reset t_timezone to 13
        t_timezone = 13;
      }
    } else if ((xT >= 215) && (xT <= 295) && (yT >= 53) && (yT <= 83)) {
      buzzer.tok();
      // DST
      t_dst = !t_dst;
  ...
  if (savetimealarm == true) {
    ...
    EEPROM.write(EEPROM_TIMEZONE, timezone+12);
    EEPROM.write(EEPROM_DST, dst ? 1 : 0);
...
void drawScreen() {
  ...
  tft.fillRect(TFT_WIDTH-3, ROW_DOOR, 3, CHAR_SIZE, TFT_BLACK);
...
void drawXXXLNum(int16_t num, int16_t x, int16_t y, boolean clear_bg) {
  drawChar(num, x, y, TFT_WHITE, TFT_BLACK, 64, 100, clear_bg, SevenSeg_XXXL_Num+4);
  yield();
}
...
t_dir get_dir(t_dir opt1, t_dir opt2, t_dir &wdir, boolean is_pacman, boolean rand2) {
  ...
  t_dir dir = (ESP8266TrueRandom.random(rand2 ? 2 : 4) == 1 ? opt1 : opt2);
